...
ClawDesk is a separate app that runs beside OpenClaw and gives a non-technical operator a human-readable control room for:
...
- ClawDesk does **not** invent fake agents, tasks, approvals, or office chatter
- if OpenClaw is not connected, the dashboard stays in `offline` mode and shows an honest empty state
- live Nova chat, live agent lists, approvals, config editing, and workspace editing only become meaningful when ClawDesk is connected to a real OpenClaw gateway on the same host

## What ClawDesk is

ClawDesk is not a static frontend only.
...
ClawDesk writes only through real control surfaces:
...
To support live file editing safely, ClawDesk should run on the same VPS or PC as OpenClaw.
...
ClawDesk performs live actions using:
...
Run ClawDesk on the same VPS as OpenClaw:

- ClawDesk bind address: `127.0.0.1`
- ClawDesk port: `3010`
...
3. Install dependencies and build ClawDesk.
4. Start ClawDesk locally and verify `/api/dashboard`.
5. Keep ClawDesk in `offline` mode until the gateway is confirmed.
6. Point ClawDesk at the live OpenClaw gateway.
...
- [docs/openclaw-integration-contract.md](./docs/openclaw-integration-contract.md) - exact integration surfaces ClawDesk depends on
- [docs/ui-gallery.html](./docs/ui-gallery.html) - current verified screenshots
- [docs/clawdesk-presentation.html](./docs/clawdesk-presentation.html) - presentation overview for operators and reviewers
...
- Run ClawDesk on the same host as OpenClaw if you want live workspace file editing.

## Repository

Public GitHub repo:

- [AKT93-LAB/openclaw-desk](https://github.com/AKT93-LAB/openclaw-desk)
